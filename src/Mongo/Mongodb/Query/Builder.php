...
//            return $results;
        ...
        else
        {
            ...
            if ($this->projections)
            {
                $columns['projection'] = array_merge($columns, $this->projections);
            }
            if ($this->limit)
                $columns["limit"] = $this->limit;

            if ($this->orders)
                $columns["sort"] = $this->orders;

            if ($this->timeout)
                $columns["maxTimeMS"] = $this->timeout;

            if ($this->offset)
                $columns["skip"] = $this->offset;

            if ($this->hint)
                $columns["projection"] = $this->hint;

            $result = $this->collection->find($wheres, $columns);
    ...
    public function insert(array $values)
    {
        ...
        // Batch insert
        $result = $this->collection->create($values, [], true);

        return (1 == (int) $result->getInsertedCount());
    }
